...
+ Taking the agricultural production regions defined by SBC
...
Prod_reg <- 
  ...
  mutate(Area = case_when(
    PO8 == 1 ~ "Götalands Södra Slättbygder", 
    PO8 == 2 ~ "Götalands Mellanbygder",
    PO8 == 3 ~ "Götalands Norra Slättbygder",
    PO8 == 4 ~ "Svealands Slätbygder",
    PO8 == 5 ~ "Götalands Skogsbygder",
    PO8 == 6 ~ "Mellersta Svergies Skogsbygder",
    PO8 == 7 ~ "Nedre Norrland",
    PO8 == 8 ~ "Övre Norrland"
  )) %>% st_set_crs(3021) %>% 
  st_transform(crs = 3006)
...
{ggplot(Prod_reg)+
  ...
  geom_sf(data = Point_counts)+
  geom_sf(data = Point_counts_sel, colour = "red")+
