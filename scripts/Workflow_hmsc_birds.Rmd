...
+ Agricultural production regions as defined by SBC
...
`public_standardrutter_koordinater.xlsx` (should be replaced by `stdruttLinjer.shp`)
...
We use the following two variable for modelling:
...
knitr::include_graphics(here("model", "figures", "resi_corr_stacked.jpg"))
```
...
read_tsv(here("data", "betapar,txt")) %>% 
  dplyr::select(one_of("bird", "fixed_effect"), starts_with("0")) %>% 
...
table %>% 
  kable(col.names = c("Species", "fixed effect", "q_2.5%", "Median", "q_97.5%"),
