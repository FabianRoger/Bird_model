...
reformat geo data and add coordinates for each point and line count
```{r}
...
+ `yr`: the year
+ `art`: the artcode.
    + artcode 000 is used to store the time at which a point has been counted and the (in minutes) which it has taken to walk a given line
...
filter(Standard_data, ! art %in% c("000", "999")) %>% # filter out non-species codes
  ...
  left_join(select(species_codes, art, englishname, genus, species, lat6)) %>% 
  left_join(Standard_data_meta_p) %>% 
  select(karta, yr, persnr, datum, timepoint, position, art, abundance, englishname, genus, species, lat6) %>% 
...
filter(Standard_data, ! art %in% c("000", "999")) %>% # filter out non-species codes
  ...
  select(karta, yr, persnr, datum, timepoint, position, prop_counted, art,
         abundance, englishname, genus, species, lat6) %>% 
  mutate_if(is.numeric, as.integer)
